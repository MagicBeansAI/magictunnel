...
impl RateLimiter {
    ...
    /// Update configuration (for dynamic config updates from dashboard)
    pub fn update_config(&self, new_config: RateLimitConfig) -> Result<()> {
        // Note: For simplicity, we'll recreate the rate limiter with new config
        // In production, you might want to preserve some state during config updates
    ...
    fn get_client_ip(&self, req: &HttpRequest) -> Option<IpAddr> {
        // Try to get real client IP from common proxy headers
        ...
        req.connection_info().peer_addr()
            .and_then(|addr| {
                ...
                let ip_part = if addr.contains('[') {
                    // IPv6 with port: [::1]:8080 -> ::1
                    addr.trim_start_matches('[').split(']').next().unwrap_or(addr)
                } else {
                    // IPv4 with port: 127.0.0.1:8080 -> 127.0.0.1
                    addr.split(':').next().unwrap_or(addr)
                };
                IpAddr::from_str(ip_part).ok()
            })
    }
...
/// Shared rate limiting middleware that uses the global rate limiter
pub struct SharedRateLimitMiddleware;

impl SharedRateLimitMiddleware {
    /// Create new shared rate limiting middleware
    pub fn new() -> Self {
        Self
    }
}
...
/// Transform implementation for Shared Actix Web middleware
impl<S, B> Transform<S, ServiceRequest> for SharedRateLimitMiddleware
where
    ...
    B: 'static,
{
    ...
    type InitError = ();
    type Transform = SharedRateLimitService<S>;
    type Future = Ready<std::result::Result<Self::Transform, Self::InitError>>;

    fn new_transform(&self, service: S) -> Self::Future {
        ok(SharedRateLimitService {
...
impl<S, B> Service<ServiceRequest> for SharedRateLimitService<S>
where
    S: Service<ServiceRequest, Response = ServiceResponse<B>, Error = ActixError> + 'static,
    S::Future: 'static,
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let global_rate_limiter = crate::web::dashboard::get_global_rate_limiter();
            
            // Check rate limiting before processing request
            let should_allow = match global_rate_limiter.read() {
                Ok(rate_limiter) => {
                    match rate_limiter.check_request(req.request()) {
                        Ok(allowed) => {
                            if allowed {
                                debug!("Request allowed by shared rate limiter: {} {}", 
                                       req.method(), req.path());
                                true
                            } else {
                                warn!("Request blocked by shared rate limiter: {} {} from {:?}", 
                                      req.method(), req.path(), 
                                      req.connection_info().peer_addr());
                                false
                            }
                        ...
                        Err(e) => {
                            warn!("Shared rate limiter error: {} - allowing request", e);
                            true // Allow request on error to avoid blocking legitimate traffic
                        }
                    }
                },
                Err(e) => {
                    warn!("Failed to acquire global rate limiter lock: {} - allowing request", e);
                    true // Allow request on lock error
                }
            };
            
            if should_allow {
                // Map successful response to left body
                let result = service.as_ref().call(req).await?;
            ...
            match rate_limiter.check_request(req.request()) {
                Ok(allowed) => {
                    if allowed {
                        ...
                        // Request is blocked by rate limiting
                        warn!("Request blocked by rate limiter: {} {} from {:?}", 
                              req.method(), req.path(), 
                              req.connection_info().peer_addr());
                ...
                Err(e) => {
                    // Rate limiter error - log and allow request to avoid blocking legitimate traffic
